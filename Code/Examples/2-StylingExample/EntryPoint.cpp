...
public:
	...
	void OnCreated(maxGUI::FormConcept* form) noexcept {
		...
		username_textbox_ = form->AddControl<>(max::Containers::MakeRectangle(50, 100, 300, 50), "");
		...
		password_textbox_ = form->AddControl<>(max::Containers::MakeRectangle(50, 150, 300, 50), "", maxGUI::TextBoxStyles::Password);

		// A Button can have the Default style, which allows the user to press Enter at any time to press the button.
		form->AddControl<>(max::Containers::MakeRectangle(50, 250, 100, 100), "Login", maxGUI::ButtonStyles::Default | maxGUI::ButtonStyles::Disabled);
	}
